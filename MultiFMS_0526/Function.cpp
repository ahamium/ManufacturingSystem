...
auto FIND_MIN_PT(Initialize InitializeSystem, vector<Pallet> PALLET_temp, MatrixMin MATRIX, std::mutex& mux) //파라미터(PALLET, PALLET_temp) 자체가 바껴서 들어와야됨  //PALLET_temp(인풋데이터)
{
	...
	*/
	...
	MATRIX.aryoper = new int**[NumPallet];
...
void New_Matrix(vector<Pallet> PALLET_temp, Initialize InitializeSystem, int PalletInSys, vector<MatrixMax> Matrix_For_Max,
	MatrixMin MATRIX_DUE, MatrixMin MATRIX, int Num_P, int P1, int P2, int P3, int P4, std::mutex& mux) //이 함수를 재사용 하고싶거든 파라미터를 통일 시키시오 반.드.시) 
{
	...
	for (int i = 0; i < PALLET_temp_public.size(); i++)
	{
		...
		for (int j = 0; j < PALLET_temp_public[i].SelecPart.size(); j++)
		{
			...
			for (int k = 0; k < 1; k++)
			{
				...
				{
					cout << "M" << PALLET_temp_public[i].SelecPart[j].Oper[OP_idx].Mac[h].Mac_No << " ";
				}
			}
		}
	...
	//B Matrix상의 최대 가공시간 VS 머신간이동시간 비교  (가공끝난 오퍼레이션 인덱스 연결)
	...
	else
	{
		...
		if (InitializeSystem.TransTime > Max_PT.temp_pallet[0].processing_time)
		{
			...
			cout << "M1 선택된 파트 및 OP : [PALLET" << Mac1_Part[0].pallet_no << "] Part" << Mac1_Part[0].part_no << " O" << Mac1_Part[0].op_no + 1 << " " << Mac1_Part[0].PT_Trans_Time << endl;
			cout << "M2 선택된 파트 및 OP : [PALLET" << Mac2_Part[0].pallet_no << "] Part" << Mac2_Part[0].part_no << " O" << Mac2_Part[0].op_no + 1 << " " << Mac2_Part[0].PT_Trans_Time << endl;
			cout << "M3 선택된 파트 및 OP : [PALLET" << Mac3_Part[0].pallet_no << "] Part" << Mac3_Part[0].part_no << " O" << Mac3_Part[0].op_no + 1 << " " << Mac3_Part[0].PT_Trans_Time << endl;
	...
	//PALLET_temp.clear();
	//PALLET_temp = B;
	//cout << "이동시간" << InitializeSystem.TransTime << endl;
	//if (InitializeSystem.TransTime > Max_PT.temp_pallet[0].processing_time)
	//{
	//	mux.lock();
	//	cout << "이동시간이 더 큼 -> 놀고있는 가장 가까운 머신 부터 납기가장 짧은 파트(오퍼레이션) 탐색" << endl;
	...
	//	/*auto Min_Due = FIND_MIN_DUE(InitializeSystem, PALLET_temp, MATRIX_DUE, B, mux);
	...
	//else  // !!!!!!!!!!!지금 에러 이동시간이 적은경우 B에서 비는 머신과 팔렛내에서 최소가공시간이 안구해지고 엄한 오퍼레이션이 선택된  문제임!!!!!!!!!!!!
	//{
	//	mux.lock();
	//	cout << "최대가공시간이 더 큼 -> 놀고있는 가장 가까운 머신부터 가공+이동시간 가장 작은 파트(오퍼레이션) 탐색" << endl;
	//	mux.unlock();

	//	/*auto Min_PT_TT = FIND_MIN_PT(InitializeSystem, PALLET_temp, MATRIX, B, mux);
	//	Mac1_Part = Min_PT_TT.MAC1_temp;
	//	Mac2_Part = Min_PT_TT.MAC2_temp;
	//	Mac3_Part = Min_PT_TT.MAC3_temp;
	//	B = Min_PT_TT.B;
...
void Move_A_to_B(int Pallet_count, vector<mac> Mac1_Part, vector<Pallet> PALLET_temp,
	Initialize InitializeSystem, int PalletInSys, vector<MatrixMax> Matrix_For_Max,
	MatrixMin MATRIX_DUE, MatrixMin MATRIX, int Num_P, int P1, int P2, int P3, int P4, std::mutex& mux) //이 함수를 재사용 하고싶거든 파라미터를 통일 시키시오 반.드.시

{
   ...
   //B Matrix에서 완료된 팔렛 삭제 
	...
	Go_F_State == 0;
	New_Matrix(PALLET_temp, InitializeSystem, PalletInSys, Matrix_For_Max,
	...
	Initialize InitializeSystem, vector<MatrixMax> Matrix_For_Max, MatrixMin MATRIX_DUE, MatrixMin MATRIX, int Num_P, int P1, int P2, int P3, int P4, std::mutex& mux)
{
	...
	if (Pallet_count > 0)  //완료된 팔렛이 하나 이상이면 
	{
		...
		Move_A_to_B(Pallet_count, Mac1_Part, PALLET_temp,
	...
	else //완료된 팔렛 없으면 
	{
		...
		New_Matrix(PALLET_temp, InitializeSystem, PalletInSys, Matrix_For_Max, MATRIX_DUE, MATRIX, Num_P, P1, P2, P3, P4, std::ref(mux));
	}
};
	...
	Initialize InitializeSystem, vector<MatrixMax> Matrix_For_Max, MatrixMin MATRIX_DUE, MatrixMin MATRIX, int Num_P, int P1, int P2, int P3, int P4, std::mutex& mux)
{
	...
	if (M1_State == 0)
	{
		...
		/*if (Go_F_State == 0)
		{
			Go_Func(PalletInSys, PALLET_temp, B, Mac1_Part, Mac2_Part, Mac3_Part,
				InitializeSystem, Matrix_For_Max, MATRIX_DUE, MATRIX, Num_P, P1, P2, P3, P4, std::ref(mux));
		}*/
	}
	else
